...
class HeatMapSwingSample extends JFrame implements ItemListener, FocusListener {
    ...
    ImageIcon[] icons;
    String[] names = { "GRADIENT_GREEN_YELLOW_ORANGE_RED", "GRADIENT_BLACK_TO_WHITE", "GRADIENT_BLUE_TO_RED",
            "GRADIENT_HEAT", "GRADIENT_HOT", "GRADIENT_MAROON_TO_GOLD", "GRADIENT_RAINBOW", "GRADIENT_RED_TO_GREEN",
            "GRADIENT_ROY" };
    Color[][] gradients = { Gradient.GRADIENT_GREEN_YELLOW_ORANGE_RED, Gradient.GRADIENT_BLACK_TO_WHITE,
            Gradient.GRADIENT_BLUE_TO_RED, Gradient.GRADIENT_HEAT, Gradient.GRADIENT_HOT,
            Gradient.GRADIENT_MAROON_TO_GOLD, Gradient.GRADIENT_RAINBOW, Gradient.GRADIENT_RED_TO_GREEN,
            Gradient.GRADIENT_ROY };

    public HeatMapSwingSample() throws Exception {
        super("Heat Map Demo");
        ...
        double[][] data = HeatMapPanel.generateSinCosData(200);
        boolean useGraphicsYAxis = true;

        // you can use a pre-defined gradient:
        panel = new HeatMapPanel(data, useGraphicsYAxis, Gradient.GRADIENT_GREEN_YELLOW_ORANGE_RED);
    ...
    private static void createAndShowGUI() throws Exception {
        HeatMapSwingSample hmSample = new HeatMapSwingSample();
        hmSample.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        hmSample.setSize(800, 600);
        hmSample.setVisible(true);
    }
    ...
    protected ImageIcon createImageIcon(String path) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (imgURL == null) {
                imgURL = Thread.currentThread().getContextClassLoader().getResource(parentDir + path);
            }
            parentDir += "../";
        }

        if (imgURL == null) {
//            System.err.println("Couldn't find file: " + path + " at " + parentDir);
            return null;
        }
